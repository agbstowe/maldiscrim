...
#'
#' @param pythonVersion Character. Python version used to create the virtual environment.
#' Default is \code{"3.10"}.
#' @param force Logical. If \code{TRUE}, removes and recreates the environment if it
#' already exists. Default is \code{FALSE}.
#' @param verbose Logical. If \code{TRUE}, prints progress messages. Default is \code{TRUE}.
#'
#' @details
#' This function only needs to be run once per machine. It installs \code{tensorflow},
#' \code{keras}, \code{numpy}, \code{pandas}, \code{scikit-learn}, \code{scipy},
#' \code{umap-learn}, \code{plotly}, \code{matplotlib}, \code{seaborn}, \code{plotnine},
#' \code{opencv-python}, \code{tf-keras-vis} and \code{xplique} into the
#' \code{"maldiscrim-env"} virtual environment, then activates it for the current R session
#' via \code{reticulate::use_virtualenv()}.
#'
#' If Python virtual environments on this machine are stored inside a OneDrive-synced folder,
#' environment creation may fail; in that case set the \code{WORKON_HOME} environment variable
#' to a non-synced folder before calling this function, e.g.
#' \code{Sys.setenv(WORKON_HOME = "C:/r-venvs")}.
#'
#' @return Invisibly returns \code{TRUE} if the environment was successfully created
#' or already existed and was activated.
#'
...
maldiscrim_install_python <- function(pythonVersion = "3.10", force = FALSE, verbose = TRUE) {
  ...
  pythonPackages <- c(
    "tensorflow", "keras", "numpy", "pandas", "scikit-learn", "scipy",
    "umap-learn", "plotly", "matplotlib", "seaborn", "plotnine",
    "opencv-python", "tf-keras-vis", "xplique"
  )
